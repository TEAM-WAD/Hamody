...
return send(msg.chat_id,msg.id,"ادري حبي انته تستغبي عليه شلون تريد تزوج نفسك ؟","md",true)
end
if tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"انكلع مني اول تالي اخذلي واحد ملطلط","md",true)
end
if Redis:sismember(Fast..msg.chat_id.."zwgat:",rep_idd) then
local rd_mtzwga = {
"هاي الصاكه مزوجه حبي ",
"شوفلك غيره مزوجه هاي ",
"متزوجه للاسف دور غيرها",
"متزوجه حبي واذا متوازي اشوفلك اخته؟",
"متزوجه هاي شسوي بيه لاخلقه ولا اخلاق"
}
return send(msg.chat_id,msg.id,Reply_Status(rep_idd,rd_mtzwga[math.random(#rd_mtzwga)]).Reply,"md",true)
else
local rd_zwag = {
" بارك الله لكم وعليكم تم زواج",
"تم زواجكم حبايب تتهنو نشالله",
"مبروك تم زواجكم حبايبي نشوف نشالله جهالكم",
"تم عقدكم نشالله اخذو راحتكم هسه بلغرفه",
"الزواج تم اتفضلو اعملو احلا واحد كلللوووششش"
}
...
-- tlaq
if text == "طلاق" or text == "خلع" or text == "تزيل زوجي" and msg.reply_to_message_id ~= 0 then
local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
local UserInfo = bot.getUser(rep_idd)
if tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"ليش متزوج نفسك وتريدني اطلك نفسك؟ ","md",true)
end
if tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"شوكت تزوجتو انتو هيو حته اطلكم؟ ","md",true)
end
if Redis:sismember(Fast..msg.chat_id.."zwgat:",rep_idd) then
...
local rd_tmtlaq = {
"تم طلاقكم للاسف",
  "تم طلاقكم اكيد ام عباس طشرتكم",
"تم الطلاق يمكن العريس مادبره",
"تم الطلاق بينكم يمكن اكو تس دخلت بينكم",
" طلكتكم هسه اني وانتي صرتي مطلقه خويه روحي هسه الزمي عده ونجبي."
}
return send(msg.chat_id,msg.id,Reply_Status(rep_idd,rd_tmtlaq[math.random(#rd_tmtlaq)]).Reply,"md",true)
else
local rd_tlaq = {
"مامتزوج انت حته تطلك",
"بايره محد ازوجها",
"محد معبره خطيه"
}
...
return send(msg.chat_id,msg.id,"   هوا بكلبك حبي شتريد اكثر من هيج ","md")
elseif tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت خبل حبي تريد ترفع نفسك فقلبك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن الخبل","md")
elseif Redis:get(Fast..msg.chat_id..rep_idd.."in_heart:") then
return send(msg.chat_id,msg.id,"للاسف هاذ بكلب واحد ثاني  ","md")
...
return send(msg.chat_id,msg.id,Reply_Status(rep_idd,"تم تنزيل من قلبك").Reply,"md",true) 
elseif tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت خبل حبي تريد تنزل نفسك من قلبك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن العميه","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."in_heart:")) ~= tonumber(senderr)then
return send(msg.chat_id,msg.id,"هو بقلبك اصلا حته انزله  ؟؟","md")
end
end
if text == "انا فقلب مين" then
if not Redis:get(Fast..msg.chat_id..senderr.."in_heart:") then
return send(msg.chat_id,msg.id,"    انته اكعد ع صفحه حبي محد طايقك","md")
...
return send(msg.chat_id,msg.id,"قلبك فارغ محد بي","md")
elseif #heart_list > 0 then
your_heart = "الناس الي بقلبك \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
...
return send(msg.chat_id,msg.id,"تم مسح الي بقلبك","md")
end
-------
-- ttgwzine
...
return send(msg.chat_id,msg.id,"ماعدنا نسوان للزواج انكلع","md")
end
...
{text = ' ماموافق', data = senderr..'/no_zw/'..member_id},
},
}
}
return send(msg.chat_id,msg.id,"جبتلك عروسه حلوه ومعدله  "..mem_tag.."  شنو رايك بهاي  ؟؟","md",false, false, false, false, reply_markup)
end
end
...
return send(msg.chat_id,msg.id,"     انتو مو مزوجين ؟","md")
elseif tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت خبل حبي تريد تتزوج نفسك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن الخبل","md")
...
return send(msg.chat_id,msg.id,"احجي يا ["..zwga_name.."](tg://user?id="..zwga_id..") زوجج رايدج ","md")
elseif not Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
return send(msg.chat_id,msg.id,"   انت سنكل حبي ","md")
end
end
if text == "زوجي" then
if Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
local zwga_id = Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")
local zwga_name = FlterBio(bot.getUser(zwga_id).first_name)
return send(msg.chat_id,msg.id,"احجي يا ["..zwga_name.."](tg://user?id="..zwga_id..") مرتك تريدك ","md")
elseif not Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
return send(msg.chat_id,msg.id,"   انتي سنكل خويه ","md")
end
end
if text == "انتي طالق" and msg.reply_to_message_id ~= 0 then 
local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
if tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت خبل حبي تريد تطلق نفسك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن الخبله","md")
...
return send(msg.chat_id,msg.id,"    هاي مومرتك علمود اطلكك يخبل","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."mtzwga:")) == tonumber(senderr) or tonumber(Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")) == tonumber(rep_idd) then
Redis:del(Fast..msg.chat_id..rep_idd.."mtzwga:")
Redis:del(Fast..msg.chat_id..senderr.."mtzwga:")
return send(msg.chat_id,msg.id,"تم طلاقكم شوفو هسه الجهال ويامن يروحون","md")
end
end
if text == "انت طالق" and msg.reply_to_message_id ~= 0 then 
local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
if tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انتي خبله يافرچه تردين تطلكين نفسك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعدي عني يبنت الخبله","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."mtzwga:")) ~= tonumber(senderr) or tonumber(Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")) ~= tonumber(rep_idd) then
return send(msg.chat_id,msg.id,"مو زوجج يخبله علمود تطلقي منو","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."mtzwga:")) == tonumber(senderr) or tonumber(Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")) == tonumber(rep_idd) then
Redis:del(Fast..msg.chat_id..rep_idd.."mtzwga:")
Redis:del(Fast..msg.chat_id..senderr.."mtzwga:")
return send(msg.chat_id,msg.id,"تم طلاقكم وانكلعو يلا وجافوني شركم  ","md")
end
end
if text == "بوت طلقني" then
if not Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then 
return send(msg.chat_id,msg.id," مامزوج انته لتلح ","md")
elseif Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
local zwg_id = Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")
local zwg_name = bot.getUser(zwg_id).first_name
Redis:del(Fast..msg.chat_id..senderr.."mtzwga:")
Redis:del(Fast..msg.chat_id..zwg_id.."mtzwga:")
return send(msg.chat_id,msg.id,"تم طلاقك من ["..FlterBio(zwg_name).."](tg://user?id="..zwg_id..")\nشوفو مين ياخذ الجهال ","md")
end
end
